    ...
    static class SPI extends SecureRandomSpi {
        ...
        protected void engineNextBytes(byte[] bytes) {
            ...
            while (offset < bytes.length) {
                ...
                // Critical native code cannot throw exception (which shouldn't happen anyway)
                // so will return false indicating error
                if (!getState().use(ptr ->generate(ptr, bytes, currentOffset, toGenerate))) {
                    throw new RuntimeCryptoException("Unknown issue when generating random data");
                }
                offset += toGenerate;
            }
        }
